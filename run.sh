...
if [ $stage -le 3 ]; then
  ...
  local/train_lms_srilm.sh \
    --train-text data/train_clean_100/text --dev-text data/dev_clean/text \
    --oov-symbol "<unk>" --words-file data/lang_nosp/words.txt \
    data/ data/srilm
  
  echo "SRI lm part completed"
fi



echo "============SRILM model build completed=========="
...
# fi
echo "============feature extracting started=========="
...
if [ $stage -le 18 ]; then
  ...
  for test in  test_other dev_clean dev_other; do
